...
extern uint8_t tft_Bahn_Cond_16[];
extern uint8_t tft_Bahn_Cond_18[];
extern uint8_t tft_Bahn_Cond_20[];
extern uint8_t tft_Bahn_Cond_24[];
extern uint8_t tft_Bahn_Cond_28[];
extern uint8_t tft_Bahn_Cond_32[];
extern uint8_t tft_Consola_14[];
extern uint8_t tft_Consola_20[];
extern uint8_t tft_Consola_28[];
	...
	else
	{
		if (font == USER_FONT)
		{
			if (load_file_font(font_file, 0) != 0)
				...
				cfont.font = tft_Bahn_Cond_16;
			else
				cfont.font = userfont;
		}
		else if (font == CONSOLA14_FONT)
			cfont.font = tft_Consola_14;
		else if (font == CONSOLA20_FONT)
			cfont.font = tft_Consola_20;
		else if (font == CONSOLA28_FONT)
			cfont.font = tft_Consola_28;
		else if (font == BAHN16_FONT)
			cfont.font = tft_Bahn_Cond_16;
		else if (font == BAHN18_FONT)
			cfont.font = tft_Bahn_Cond_18;
		else if (font == BAHN20_FONT)
			cfont.font = tft_Bahn_Cond_20;
		else if (font == BAHN28_FONT)
			cfont.font = tft_Bahn_Cond_28;
		else if (font == BAHN32_FONT)
			cfont.font = tft_Bahn_Cond_32;
		else
			//cfont.font = tft_DefaultFont;
			cfont.font = tft_Bahn_Cond_16;
...
void TFT_print(char *st, int x, int y)
{
	...
	if ((x >= LASTX) && (x < LASTY))
		...
		x += dispWin.x1;  //add to window

	if (y >= LASTY)
		y = TFT_Y + (y - LASTY);
	else if (y > CENTER)
		y += dispWin.y1;  //add to window
	...
	if (x == RIGHT)
		//x = dispWin.x2 - tmpw + dispWin.x1;		// wrong tmpw=10 x1=80 x2=160 -> x =230 !! (correct = 150)
		x = dispWin.x2 - tmpw;  //x1 does not matter for RIGHT!! should be correct         
	else if (x == CENTER)  
		x = (((dispWin.x2 - dispWin.x1 + 1) - tmpw) / 2) + dispWin.x1; //correct

	if (y == BOTTOM)
		//y = dispWin.y2 - fh + dispWin.y1; //wrong
		y = dispWin.y2 - fh; // y1 is irrelevant here
	else if (y == CENTER)
		// y = (((dispWin.y2 - dispWin.y1 + 1) - (fh / 2)) / 2) + dispWin.y1;
		y = (((dispWin.y2 - dispWin.y1 + 1) - fh) / 2) + dispWin.y1;
